...
def parse_args():
    ...
    ###### only for test ######
    # args.train_file = 'HW2/data/train.json'
    # args.validation_file = 'HW2/data/valid.json'
    # args.max_source_length = 512
    # args.pad_to_max_length = True
    # args.model_name_or_path = 'google/mt5-small'
    # args.text_column = 'maintext'
    # args.summary_column = 'title'
    # args.gradient_accumulation_steps = 2
    # args.num_warmup_steps = 300
    # args.output_dir = 'HW2_training_result'
    # args.seed = 228
...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                rouge_scores = rouge.get_scores(generated_text, labels)
                rouge_l_score = rouge_scores[0]['rouge-l']['f']
                reward = rouge_l_score
                coefficient = 1
                loss += coefficient * (-reward)
                #####
    ...
    if args.output_dir is not None:
        ...
        json.dump({'curve': curve_list}, open(os.path.join(args.output_dir, "training_curve.json"), 'w'), indent=2)
        if accelerator.is_main_process:
            tokenizer.save_pretrained(args.output_dir)
            if args.push_to_hub:
